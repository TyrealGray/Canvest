...
fastify.route({
	...
	handler: async (req, reply) => {
		try {
			...
			const imageDataBase64 = imageDataURL.replace(/^data:image\/png;base64,/, "");

			const imagePath = path.join(process.cwd(),'canvest','autoShot',`${fileName}.png`);
			let pass = false,
			...
			if(!fs.existsSync(imagePath)){
				...
				const diff = pixelmatch(PNG.sync.read(buffer).data, snapShot.data, diffImage.data, width, height);
			...
			const dataURL = cachedImageBuffer? cachedImageBuffer.toString('base64') : null;
